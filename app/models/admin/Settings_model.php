...
class Settings_model extends CI_Model
{
    ...
    public function addBusiness_location($data)
    {
        if ($this->db->insert('business_location', $data)) {
            return true;
        }
        return false;
    }
    ...
    public function deleteBusiness_location($id)
    {
        if ($this->db->delete('business_location', ['id' => $id])) {            
            return true;
        }
        return false;
    }
    ...
    public function getAllBusiness_location()
    {
        $q = $this->db->get('business_location');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getBusiness_locationByID($id)
    {
        $q = $this->db->get_where('business_location', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function updateBusiness_location($id, $data = [])
    {
        $this->db->where('id', $id);
        if ($this->db->update('business_location', $data)) {
            return true;
        }
        return false;
    }
