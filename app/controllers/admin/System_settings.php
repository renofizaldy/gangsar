...
class system_settings extends MY_Controller
{
    ...
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang('code'), 'trim|is_unique[business_location.code]|required');
        $this->form_validation->set_rules('name', lang('name'), 'required');
        $this->form_validation->set_rules('address', lang('address'), 'required');
        $this->form_validation->set_rules('userfile', lang('map_image'), 'xss_clean');
        ...
        if ($this->form_validation->run() == true) {
            if ($_FILES['userfile']['size'] > 0) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('message', $error);
                    admin_redirect('system_settings/business_location');
                }
        ...
        } elseif ($this->input->post('add_business_location')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/business_location');
        }

        if ($this->form_validation->run() == true && $this->settings_model->addBusiness_location($data)) {
            $this->session->set_flashdata('message', lang('business_location_added'));
            admin_redirect('system_settings/business_location');
        } else {
            $this->data['error']        = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['price_groups'] = $this->settings_model->getAllPriceGroups();
            $this->data['modal_js']     = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_business_location', $this->data);
        }
    }
    ...
    public function delete_business_location($id = null)
    {
        if (!$id) {
            $this->sma->send_json(['error' => 1, 'msg' => lang('id_not_found')]);
        }
        if ($this->settings_model->deleteBusiness_location($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => lang('business_location_deleted')]);
        }
    }
    ...
    public function edit_business_location($id = null)
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang('code'), 'trim|required');
        $wh_details = $this->settings_model->getBusiness_locationByID($id);
        if ($this->input->post('code') != $wh_details->code) {
            $this->form_validation->set_rules('code', lang('code'), 'required|is_unique[business_location.code]');
        }
        $this->form_validation->set_rules('address', lang('address'), 'required');
        $this->form_validation->set_rules('map', lang('map_image'), 'xss_clean');
        ...
        } elseif ($this->input->post('edit_business_location')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/business_location');
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateBusiness_location($id, $data)) { //check to see if we are updateing the customer
            $this->session->set_flashdata('message', lang('business_location_updated'));
            admin_redirect('system_settings/business_location');
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['business_location']    = $this->settings_model->getBusiness_locationByID($id);
            $this->data['price_groups'] = $this->settings_model->getAllPriceGroups();
            $this->data['id']           = $id;
            $this->data['modal_js']     = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_business_location', $this->data);
        }
    }
    ...
    public function getBusiness_location()    
    {
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('business_location')}.id as id, map, code, {$this->db->dbprefix('business_location')}.name as name, '' as price_group, phone, email, address")
            ->from('business_location')            
            ->add_column('Actions', "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_business_location/$1') . "' class='tip' title='" . lang('edit_business_location') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang('delete_business_location') . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_business_location/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');
    ...
    public function business_location_actions()
    {
        $this->form_validation->set_rules('form_action', lang('form_action'), 'required');
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deleteBusiness_location($id);
                    }
                    $this->session->set_flashdata('message', lang('business_location_deleted'));
                    redirect($_SERVER['HTTP_REFERER']);
                }
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('business_location'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('code'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('name'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('address'));
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('city'));
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $wh = $this->settings_model->getbusiness_locationByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $wh->code);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $wh->name);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $wh->address);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $wh->city);
                    ...
                    $this->excel->getActiveSheet()->getColumnDimension('D')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical('center');
                    $filename = 'business_location_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
            } else {
    ...
    public function business_location()
    {
        ...
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('business_location')]];
        $meta = ['page_title' => lang('business_location'), 'bc' => $bc];
        $this->page_construct('settings/business_location', $meta, $this->data);
    }
