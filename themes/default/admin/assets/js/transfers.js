...
function loadItems() {
    if (localStorage.getItem('toitems')) {
        ...
        var tfoot =
            ...
            '</th>';
        if (site.settings.tax1 == 1) {
            tfoot += '<th class="text-right"><span class="totaltransfer">' + formatMoney(product_tax) + '</span></th>';
        }
        tfoot +=
            '<th class="text-right"><span class="totaltransfer">' +
            formatMoney(total) +
            '</span></th><th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
